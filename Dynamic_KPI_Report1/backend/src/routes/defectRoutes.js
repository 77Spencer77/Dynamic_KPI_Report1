...
router.get('/', async (req, res) => {
  try {
    ...
    const responseFinal = await fetch(
      `https://product-jira.ariba.com/rest/api/2/search?jql=${jqlQueryEncoded}&startAt=0&maxResults=${total}&fields=summary,created,labels,customfield_16500,customfield_10300,fixVersions`,
      {
        headers: {
          Authorization: `${token}`,
          Accept: 'application/json',
