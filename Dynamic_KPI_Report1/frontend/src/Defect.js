...
const Defect = () => {
  ...
  useEffect(() => {
    const readGoalsContent = async () => {
      try {
        const response = await fetch("/Defect.txt");
        const content = await response.text();
      ...
      ["P1", "P2", "P3", "P4"].forEach((priority) => {
        const filteredIssues = sortedIssues.filter(
          (issue) =>
            ...
            (!issue.fields.customfield_10300 ||
              issue.fields.customfield_10300[0]?.value !== "Security Impact")
        );
...
<div className="container d-flex align-items-center justify-content-center">
  {isLoading ? (
    ...
    <div>
          <h2>Total Internal Defects: {totalAllIssues}</h2>
          <h4>
